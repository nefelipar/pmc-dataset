...
def clean(s: str) -> str:
    ...
    # 1) Αφαιρεί κενά πριν από στίξη (π.χ. Plasmodium , inc → Plasmodium, inc)
    s = re.sub(r"\s+([,.;:!?%])", r"\1", s)

    # 2) Προσθέτει κενό μετά από στίξη, αν δεν υπάρχει ήδη (π.χ. Plasmodium,inc → Plasmodium, inc)
    s = re.sub(r"([,;:!?])([^\s)\]\}»”’])", r"\1 \2", s)

    # 3) Μην αφήνεις κενό πριν από κλείσιμο παρενθέσεων, εισαγωγικών κλπ
    s = re.sub(r"\s+([)\]\}»”’])", r"\1", s)

    # 4) Μην αφήνεις κενό αμέσως μετά από άνοιγμα παρενθέσεων, εισαγωγικών, αγκύλων
    s = re.sub(r"([(\[\{«“‘])\s+", r"\1", s)

    # 5) Αριθμοί με κόμμα (π.χ. 5,400) να παραμένουν σωστοί χωρίς να σπάνε σε 5, 400
    s = re.sub(r"(?<=\d),(?=\d)", ",", s)  # Κρατάει το κόμμα ως ενσωματωμένο στον αριθμό

    # 6) Ενδο-λέξης hyphen (π.χ. gene-name) να μένει κολλητό
    s = re.sub(r"(?<=\w)\s*-\s*(?=\w)", "-", s)  # Κρατάει το hyphen κολλητό

    # 7) Ειδικός κανόνας για το en dash (–): Μην αφήνεις κενό γύρω του
    s = re.sub(r"\s*–\s*", "–", s)  # Κρατάει το en dash χωρίς κενό γύρω του

    # 8) Δεν αφήνουμε κενό μετά από "=", όπως σε "x = 5"
    s = re.sub(r"\s*=\s*", "=", s)

    # 9) Καθάρισε διπλά κενά που έμειναν
